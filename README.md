...
- Na CLI publica atual, a conclusao normal da fase e aplicada por `phase.completion` quando `next` executa, incluindo auto-close na fase final. `eaw next` e a unica rota de fechamento de lifecycle; nao existe comando separado de fechamento standalone.
