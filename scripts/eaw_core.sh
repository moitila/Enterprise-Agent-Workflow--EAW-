...
eaw_capture_git_snapshot() {
	local card="$1"
	local outdir="$2"
	ensure_dir "$outdir/provenance"
	local snapshot_file="$outdir/provenance/git_snapshot.md"
	{
		printf "# Git Snapshot - %s\n\n" "$card"
		printf "| Repo | Path | Branch | Commit | Dirty |\n"
		printf "|------|------|--------|--------|-------|\n"
	} > "$snapshot_file"
	local dirty_details=""
	local entry repo_key repo_path repo_role
	for entry in "${REPO_ENTRIES[@]}"; do
		IFS='|' read -r repo_key repo_path repo_role <<< "$entry"
		local status branch commit dirty dirty_out err_msg
		if [[ ! -d "$repo_path" ]]; then
			status="missing"
			branch="-"
			commit="-"
			...
			err_msg=""
			branch=$(git -C "$repo_path" rev-parse --abbrev-ref HEAD 2>&1) \
				|| { err_msg="branch: $branch"; branch="-"; }
			commit=$(git -C "$repo_path" rev-parse HEAD 2>&1) \
				|| { err_msg="${err_msg} commit: $commit"; commit="-"; }
			dirty_out=$(git -C "$repo_path" status --porcelain 2>&1) \
				|| { err_msg="${err_msg} status: $dirty_out"; dirty_out=""; }
			if [[ -n "$err_msg" ]]; then
				status="error"
				dirty="unknown"
			else
				status="ok"
				[[ -n "$dirty_out" ]] && dirty="dirty" || dirty="clean"
			fi
		fi
		printf "| %s | %s | %s | %s | %s |\n" \
			"$repo_key" "$repo_path" "$branch" "$commit" "$dirty" >> "$snapshot_file"
		if [[ "$status" == "ok" && "$dirty" == "dirty" ]]; then
			dirty_details+="### ${repo_key}\n\`\`\`\n${dirty_out}\n\`\`\`\n\n"
	...
	done
	{
		...
		if [[ -n "$dirty_details" ]]; then
			printf "%b" "$dirty_details"
		else
			printf "_No dirty details._\n"
		fi
	} >> "$snapshot_file"
	return 0
}

eaw_generate_context_bundle() {
	local card="$1"
	local card_dir="$2"
	local current_phase="$3"
	local next_phase="$4"
	local track_id="$5"
	# $6 track_file: recebido mas não usado diretamente
	local bundle_path="$card_dir/runtime/context_bundle_${next_phase}.md"
	local next_phase_file="$EAW_TRACKS_DIR/${track_id}/phases/${next_phase}.yaml"
	local ts
	ts="$(utc_timestamp)"

	# H1: populate REPO_ENTRIES[] from repos.conf
	phase_resolve_repos

	# H7: fail-soft — create runtime/ dir
	if ! ensure_dir "$card_dir/runtime" 2>/dev/null; then
		printf "[warning] context bundle not generated: cannot create runtime dir\n" >&2
		return 0
	fi

	# Build repos section (H5: inline git capture)
	local repos_section
	repos_section="$(
		printf "| Repo | Path | Role | Branch | Commit | Dirty |\n"
		printf "|------|------|------|--------|--------|-------|\n"
		if [[ ${#REPO_ENTRIES[@]} -eq 0 ]]; then
			printf "[warning] no repos resolved from REPOS_CONF\n" >&2
		else
			local entry _repo_key _repo_path _repo_role _branch _commit _dirty_raw _dirty
			for entry in "${REPO_ENTRIES[@]}"; do
				IFS='|' read -r _repo_key _repo_path _repo_role <<< "$entry"
				_branch="$(git -C "$_repo_path" rev-parse --abbrev-ref HEAD 2>/dev/null)" \
					|| { printf "[warning] git query failed for repo: %s\n" "$_repo_key" >&2; _branch="-"; }
				_commit="$(git -C "$_repo_path" rev-parse HEAD 2>/dev/null)" \
					|| { printf "[warning] git query failed for repo: %s\n" "$_repo_key" >&2; _commit="-"; }
				_dirty_raw="$(git -C "$_repo_path" status --porcelain 2>/dev/null)" \
					|| { printf "[warning] git query failed for repo: %s\n" "$_repo_key" >&2; _dirty_raw=""; }
				[[ -n "$_dirty_raw" ]] && _dirty="dirty" || _dirty="clean"
				printf "| %s | %s | %s | %s | %s | %s |\n" \
					"$_repo_key" "$_repo_path" "$_repo_role" "$_branch" "$_commit" "$_dirty"
			done
		fi
	)"

	# Build artifacts section (H6: via subshell)
	local artifacts_section
	if [[ -f "$next_phase_file" ]]; then
		artifacts_section="$(eaw_phase_completion_render_artifact_status_checklist \
			"$card" "$card_dir" "$next_phase" "$next_phase_file" 2>/dev/null)"
	else
		artifacts_section="[warning] phase file not found: $next_phase_file"
	fi

	# Sovereign contracts existence check
	local lock_status plan_status
	[[ -f "$card_dir/implementation/00_scope.lock.md" ]] && lock_status="present" || lock_status="absent"
	[[ -f "$card_dir/implementation/10_change_plan.md" ]] && plan_status="present" || plan_status="absent"

	# Write bundle (H7: fail-soft on write error)
	{
		printf "# Context Bundle — %s / %s\n\n" "$card" "$next_phase"
		printf "_This is a derived/rendered view generated by \`eaw next\`. Not a sovereign source._\n\n"
		printf "Generated at: %s\n" "$ts"
		printf "Card ID: %s\n" "$card"
		printf "Track: %s\n" "$track_id"
		printf "Current phase (completed): %s\n" "$current_phase"
		printf "Next phase: %s\n\n" "$next_phase"
		printf "EAW_WORKDIR: %s\n" "${EAW_WORKDIR:-}"
		printf "OUT_DIR: %s\n" "$(dirname "$card_dir")"
		printf "CARD_DIR: %s\n\n" "$card_dir"
		printf "## Repositories\n\n%s\n\n" "$repos_section"
		printf "## Required Artifacts (next phase: %s)\n\n%s\n\n" "$next_phase" "$artifacts_section"
		printf "## Active Sovereign Contracts\n\n"
		printf "| File | Status |\n|------|--------|\n"
		printf "| implementation/00_scope.lock.md | %s |\n" "$lock_status"
		printf "| implementation/10_change_plan.md | %s |\n\n" "$plan_status"
		printf "## Write Policy\n\n"
		printf "_Write policy here is descriptive only. Enforcement remains unchanged._\n"
		printf "_Allowlist source: implementation/00_scope.lock.md (when present)._\n"
		printf "_Write scope: %s_\n" "$card_dir"
	} > "$bundle_path" || {
		printf "[warning] context bundle not generated: write error\n" >&2
		return 0
	}
...
# Parse phase.skills list from phase YAML.
# Prints one skill name per line, or empty when absent.
eaw_yaml_phase_skills() {
	local file="$1"
	awk '
		function trim(s) {
			sub(/^[[:space:]]+/, "", s)
		...
		/^phase:[[:space:]]*$/ { in_phase=1; next }
		in_phase && /^[^[:space:]]/ { in_phase=0; in_skills=0 }
		in_phase && /^  skills:[[:space:]]*$/ { in_skills=1; next }
...
# Generate agent bundle for the next phase.
# Reads phase.skills from phase YAML, resolves via skills/registry.yaml,
# writes $card_dir/runtime/agent_bundle_<next_phase>.md.
# Returns 0 on success, 1 on error (skill not found or registry absent).
eaw_generate_agent_bundle() {
	local card="$1"
	local card_dir="$2"
	local next_phase="$3"
	local track_id="$4"
	local bundle_path="$card_dir/runtime/agent_bundle_${next_phase}.md"
	local next_phase_file="$EAW_TRACKS_DIR/${track_id}/phases/${next_phase}.yaml"
	...
	if [[ ! -f "$registry_file" ]]; then
		printf "[error] agent bundle: skills/registry.yaml not found at %s\n" "$registry_file" >&2
		return 1
	fi

	# Build effective skill list: eaw_workspace always first
	local declared_skills=()
	if [[ -f "$next_phase_file" ]]; then
		while IFS= read -r skill; do
			[[ -n "$skill" ]] && declared_skills+=("$skill")
		done < <(eaw_yaml_phase_skills "$next_phase_file")
	fi

	local effective_skills=("eaw_workspace")
	local s
	for s in "${declared_skills[@]}"; do
		[[ "$s" == "eaw_workspace" ]] && continue
		effective_skills+=("$s")
	done

	# Resolve each skill via registry
        local -a skill_keys=()
        local -a skill_paths=()
        for s in "${effective_skills[@]}"; do
                local skill_file
                ...
                if [[ -z "$skill_file" ]]; then
                        printf "[error] agent bundle: skill '%s' not found in registry\n" "$s" >&2
                        return 1
                fi
                skill_keys+=("$s")
                skill_paths+=("$EAW_ROOT_DIR/$skill_file")
	done

	# Ensure runtime dir
	if ! ensure_dir "$card_dir/runtime" 2>/dev/null; then
		printf "[warning] agent bundle not generated: cannot create runtime dir\n" >&2
		return 0
	fi

	# Write bundle
	{
		printf "# Agent Bundle — %s / %s\n\n" "$card" "$next_phase"
		printf "_This is a derived/rendered view generated by \`eaw next\`. Not a sovereign source._\n\n"
		printf "Generated at: %s\n\n" "$ts"
		printf "## Effective Skills\n\n"
		local i
		for i in "${!skill_keys[@]}"; do
			printf "### %s\n\n" "${skill_keys[$i]}"
			if [[ -f "${skill_paths[$i]}" ]]; then
				cat "${skill_paths[$i]}"
				printf "\n\n"
			else
				printf "_Skill file not found: %s_\n\n" "${skill_paths[$i]}"
			fi
		done
	} > "$bundle_path" || {
		printf "[warning] agent bundle not generated: write error\n" >&2
		return 0
	}
	return 0
}
