...
eaw_repo_ai_passive_inventory() {
	...
	local target_file="$repo_path/.github/copilot-instructions.md"

	# CA2: arquivo ausente → silêncio total
	if ! test -f "$target_file"; then
		return 0
	fi

	# CA1: arquivo detectado → materializar inventory
	ensure_dir "$card_dir/context/repo_ai"

	local size_bytes status_line
	if size_bytes="$(wc -c < "$target_file" 2>/dev/null)"; then
		size_bytes="${size_bytes// /}"
		status_line="detected"
	else
		# H9: wc -c falhou → detected_with_warning
		size_bytes="unknown"
		status_line="detected_with_warning"
	fi

	local ts
	ts="$(utc_timestamp)"

	cat >"$card_dir/context/repo_ai/00_inventory.md" <<EOF
# Repo AI Inventory — ${card}

**Card:** ${card}
**Phase:** ${phase_id}
**Repo:** ${repo_key} (${repo_path})
**Detected at:** ${ts}
...
- Status: not scanned (requires EAW_REPO_AI_CONTEXT=1)
...
Passive inventory only. To enable full materialization, set EAW_REPO_AI_CONTEXT=1.
EOF

	# CA6: registrar provenance com kind=repo_ai
	# $2 = $EAW_OUT_DIR (não $card_dir) — H3/E02
	eaw_context_provenance_append \
		"$card" \
		"$EAW_OUT_DIR" \
		"$phase_id" \
		"repo_ai" \
		"github_copilot_passive_v1" \
		"$repo_path" \
		".github" \
		"1" \
		"copilot-instructions.md" \
		"github_copilot_passive_v1" \
		"out/$card/context/repo_ai" || true
