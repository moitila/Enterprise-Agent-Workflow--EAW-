...
prompt_is_known_section() {
	local section="$1"
	case "$section" in
	RUNTIME_ENVIRONMENT | GLOBAL_CONTEXT | WORKSPACE_SOURCE | ROLE | OBJECTIVE | DEFINITIONS | INPUT | OUTPUT | OUTPUT_STRUCTURE | CONTRACT_NOTES | WARNINGS_POLICY | READ_SCOPE | WRITE_SCOPE | HANDOFF_PROTOCOL | FORBIDDEN | FAIL_CONDITIONS | RULES | CONTEXT_USAGE | SPIKE_MODE_CONDITIONAL | HANDOFF_CODE_EMISSION | ONBOARDING_ENFORCEMENT_V1 | SIMPLE_ALIGNMENT_DETECTION | TRIVIAL_SCOPE_DETECTION | DOMINANTE | ROOT_CAUSE_CONFIRMED | HYPOTHESES_SKIP_DETECTION)
		return 0
		;;
	esac
	return 1
