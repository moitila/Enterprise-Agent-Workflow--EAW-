...
- Para cada arquivo original de feedback no corpus, verificar idempotencia e escrever
  ...
  <YYYY-MM-DDTHH:MM:SS> | agente=${CARD} | track=feedback_review | card=${CARD} |
  fase=backlog_finalization | resultado=<RESULTADO>
...
- ${CARD_DIR}/corpus/00_manifest.md
- ${CARD_DIR}/corpus/10_inventory.md
- ${EAW_WORKDIR}/ci_feedback/<track_de_origem>/<fase>/<arquivo> (verificar idempotencia antes do append)
...
- EXCECAO OPERACIONAL AUTORIZADA: rodape de cada arquivo em
  ${EAW_WORKDIR}/ci_feedback/<track_de_origem>/<fase>/<arquivo> (append idempotente)
...
| <ISO> | <path> | <BACKLOG:BL-N / NO_ACTION / ...> | card=<CARD>|fase=backlog_finalization | executado / ja_existia / erro |
---
...
- Esta e a fase final; nao emitir 20_handoff.json de transicao.
...
- Falhar se 62_validation_report.md nao distinguir smoke estrutural de execucao substantiva.
- Falhar se 61_appends_log.md nao registrar todos os arquivos de feedback do corpus.
