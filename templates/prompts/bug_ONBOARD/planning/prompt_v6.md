...
- Quando `hypotheses` nao foi pulado: a hipotese dominante identificada em `30_hypotheses.md` deve
  aparecer explicitamente em `Hipotese(s) Selecionada(s)`.
  Quando `hypotheses` foi pulado via `ROOT_CAUSE_CONFIRMED`: a hipotese dominante deve ser derivada
  das evidencias de `20_findings.md` e aparecer explicitamente em `Hipotese(s) Selecionada(s)`.
