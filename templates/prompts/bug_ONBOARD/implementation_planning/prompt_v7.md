...
- Ler somente:
  ...
  - `{{CARD_DIR}}/context`
  - `{{EAW_WORKDIR}}/context_sources/onboarding/<resolved_repo_key>/` (autorizado por ONBOARDING CONTEXT)
...
- VALIDACOES FINAIS:
  - Garantir que os artefatos finais nao contenham como tokens de template EAW nao resolvidos (ex: marcador de dupla-chave maiusculo ou tag angular residual).
...
- Falhar em qualquer tentativa de leitura fora de `{{CARD_DIR}}`, `{{CARD_DIR}}/investigations` e `{{CARD_DIR}}/context`.
  - EXCECAO: a leitura de `{{EAW_WORKDIR}}/context_sources/onboarding/<resolved_repo_key>/` e
    autorizada por ONBOARDING CONTEXT e nao constitui violacao de escopo.
