...
- Produzir `10_phase_output.json` com o envelope resumido da fase.
- Produzir `20_handoff.json` com o code de handoff correto para consumo por `planning`.
...
- Confirmar ao final que somente `30_hypotheses.md`, `10_phase_output.json` e `20_handoff.json` foram escritos.
- Apos escrever `30_hypotheses.md`, gerar `10_phase_output.json` com envelope deterministico:
  ```json
  {"phase_id":"hypotheses","status":"completed","summary":"<resumo>","facts":{"hypothesis_count":<N>,"has_dominant":true|false,"all_informational":true|false}}
  ```
- Em seguida, gerar `20_handoff.json`:
  - se `all_informational: true`, emitir:
    ```json
    {"from_phase":"hypotheses","status":"completed","messages":[],"codes":["NO_DOMINANT_HYPOTHESIS"]}
    ```
  - caso contrario, emitir:
    ```json
    {"from_phase":"hypotheses","status":"completed","messages":[],"codes":[]}
    ```
- Nao reutilizar `INFORMATIONAL_ONLY` para representar ausencia de hipotese dominante.
