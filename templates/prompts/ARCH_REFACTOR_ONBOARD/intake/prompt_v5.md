...
- GENERATING `## Repositório principal de onboarding` (MT01A — MANDATORY):
  When producing `{{CARD_DIR}}/investigations/00_intake.md`, you MUST:
  1. Include section `## Repositório principal de onboarding` with the single resolved repo key.
  2. The value MUST be the exact key from `TARGET_REPOS` that was resolved as `resolved_repo_key`.
  3. If `## Repositórios relacionados` is also emitted, the repo key in step 2 MUST NOT
     appear in that list (MT01B).
  4. Omitting or leaving this section empty is a phase completion blocker — STOP and report.
...
SIMPLE_ALIGNMENT_DETECTION

Apos produzir `00_intake.md`, avaliar se o card atende criterios de alinhamento simples. O card tem alinhamento simples quando TODOS os criterios abaixo forem atendidos simultaneamente:
...
Se o card tiver alinhamento simples:

1. Registrar `## Sinal de Skip` ao final de `00_intake.md` com valor `SIMPLE_ALIGNMENT`
2. Os sinais existentes (`skip_hypotheses`, `planning_mode`) permanecem obrigatorios e independentes deste sinal

Se o card NAO tiver alinhamento simples:

1. Nao registrar `Sinal de Skip`
2. O fluxo normal continua com findings
