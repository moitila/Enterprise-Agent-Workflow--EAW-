...
REPO_AI_CONTEXT_GUIDANCE
- Verificar se o arquivo `{{EAW_WORKDIR}}/context_sources/onboarding/<repo_key>/repo_ai_context.md` existe.
- Se existir:
  - Ler o conteudo de `{{EAW_WORKDIR}}/context_sources/onboarding/<repo_key>/repo_ai_context.md` antes de aplicar regras operacionais.
  - Incluir secao `## Repo AI Context Guidance` em `80_execution_contract.md` com:
    - Path da fonte: `{{EAW_WORKDIR}}/context_sources/onboarding/<repo_key>/repo_ai_context.md`
    - Sumario funcional das regras derivadas para agentes EAW (nao dump literal — extrair as regras mais relevantes)
    - Instrucao explicita ao executor sobre como aplicar as regras no card
  - Incluir secao `## Repo AI Context Guidance` em `81_agent_quickstart.md` com:
    - Instrucao de leitura: ler `repo_ai_context.md` como primeiro passo do quickstart
    - Resumo dos comportamentos esperados derivados das fontes IA nativas do repo
- Se nao existir:
  - Continuar normalmente sem incluir secao de AI context.
  - Nao emitir warning nem error por ausencia do arquivo.
  - O repo simplesmente nao tem fontes IA nativas detectadas (skip via NO_AI_CONTEXT_FOUND ou fase anterior nao executada).
