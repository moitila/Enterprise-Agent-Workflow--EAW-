...
- Do not skip this step. If the directory is absent or empty, report as a blocker before proceeding.
...
- Interpretar consumo de onboarding por referencia como uso exclusivo da superficie de contexto injetada pelo runtime, especialmente o bloco `DYNAMIC_CONTEXT` definido em INPUT, sem exigir copia para `out/<CARD>/` e sem instruir leitura de `{{CARD_DIR}}/context/onboarding/` como pre-condicao.
