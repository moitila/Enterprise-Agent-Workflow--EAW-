...
- Tratar o contrato `deterministic_baseline_v1` como superficie canonica desta fase, sem inferir sucesso a partir de materializacao parcial.

CONTRACT_NOTES
- O contrato observado desta fase inclui quatro artefatos de saida em `context/dynamic/`.
- `40_warnings.md` e opcional apenas quando nao ha warnings ou truncamentos; se houver limite atingido, descarte, truncamento ou desvio registravel, o arquivo passa a ser obrigatorio.
- A ordenacao deterministica deve permanecer `score desc` com desempate por `path asc`.
- Ausencia de marcadores `intake_*` e lacuna de provenance devem ser tratadas como contexto nao bloqueante, nunca como licenca para alterar o contrato.
...
- `40_warnings.md`: warnings nao bloqueantes e lacunas observadas; ausente apenas quando nao houver warnings, truncamentos ou desvios registraveis.
...
- Quando um warning ou truncamento ocorrer, escrever `40_warnings.md` com o limite atingido, a contagem de itens descartados e o ponto de interrupcao.
