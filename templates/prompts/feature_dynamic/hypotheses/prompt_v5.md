...
- Onboarding MUST help identify existing patterns and hypothesis candidates
...
- Every hypothesis MUST still be backed by concrete evidence (file + command + snippet)
...
- Interpretar consumo de onboarding por referencia como uso exclusivo da superficie de contexto injetada pelo runtime, especialmente o bloco de onboarding definido em INPUT, sem exigir copia para `out/<CARD>/` e sem instruir leitura de `{{CARD_DIR}}/context/onboarding/` como pre-condicao.
- VALIDACOES FINAIS:
