...
DEFINITIONS
- Consumo de onboarding por referencia: usar onboarding somente pela superficie de contexto injetada pelo runtime, especialmente `{{CONTEXT_BLOCK}}`, sem exigir copia para `out/<CARD>/`, sem instruir leitura de `{{CARD_DIR}}/context/onboarding/` como pre-condicao e sem hardcode de paths globais.
- Preservar `dynamic_context`: manter intacta a separacao entre onboarding estatico e `dynamic_context` por card; nao propor mudanca de comportamento, contrato ou persistencia para `context/dynamic/` alem do que for necessario como guardrail de nao-regressao.
...
- Em `Hipotese(s) Selecionada(s)`, listar identificadores `H[0-9]+` extraidos de `30_hypotheses.md`, incluindo obrigatoriamente a hipotese dominante.
