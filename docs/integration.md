...
The declarative lifecycle remains track-based: the runtime stores `card_state.track_id` and `current_phase`, resolves the official workflow from `tracks/<track>/track.yaml`, and advances the card with `./scripts/eaw next <CARD>` using `track.transitions`.

## Aggregated prompt flow (compatibility)

`intake -> analyze -> implement`

This aggregated prompt flow remains available only as a deprecated compatibility path. Prefer `./scripts/eaw next <CARD>` for the primary lifecycle interface and plan migration before the `v1.0` removal target.

- `intake`: deprecated compatibility wrapper; generates `intake_agent_prompt.round_<N>.md` in `<OUT_DIR>/<CARD>/investigations/` and is planned for removal in `v1.0`.
- `analyze`: deprecated compatibility wrapper; generates `findings_agent_prompt.md`, `hypotheses_agent_prompt.md`, and `planning_agent_prompt.md` in `<OUT_DIR>/<CARD>/investigations/` and is planned for removal in `v1.0`.
- `implement`: deprecated compatibility wrapper; creates implementation scaffolds in `<OUT_DIR>/<CARD>/implementation/` and generates canonical prompt artifacts (`implementation_planning_agent_prompt.md`, `implementation_executor_agent_prompt.md`) in `<OUT_DIR>/<CARD>/investigations/` with compatibility mirrors in `<OUT_DIR>/<CARD>/implementation/`; planned removal in `v1.0`.
- `planning` and `executor` remain prompt phases generated by `analyze`/`implement`; they are not top-level CLI subcommands.
- This flow is the aggregated prompt pipeline used for AI-assisted execution and compatibility; it does not replace the declarative lifecycle driven by `track`, `current_phase`, and `track.transitions`.
