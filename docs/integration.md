...
- `intake`: deprecated compatibility wrapper; materializes its prompt only in `<OUT_DIR>/<CARD>/prompts/intake.md` and is planned for removal in `v1.0`.
- `analyze`: deprecated compatibility wrapper; materializes its prompts only in `<OUT_DIR>/<CARD>/prompts/` and is planned for removal in `v1.0`.
- `implement`: deprecated compatibility wrapper; creates implementation scaffolds in `<OUT_DIR>/<CARD>/implementation/` and materializes its prompts only in `<OUT_DIR>/<CARD>/prompts/`; planned removal in `v1.0`.
