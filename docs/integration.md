...
Ensure the default prompt tree exists in workspace templates so the declared phases can resolve their prompt bindings:
...
The seeded tree starts at `prompt_v1.*`, and runtime selection is resolved by `ACTIVE` in each phase directory.
...
The declared prompt phases materialize through the workflow contract and `./scripts/eaw next <CARD>`. They do not require public `intake`, `analyze`, or `implement` subcommands.

- `planning` and `executor` remain prompt phases generated by the workflow; they are not top-level CLI subcommands.
- This flow does not replace the declarative lifecycle driven by `track`, `current_phase`, and `track.transitions`.
