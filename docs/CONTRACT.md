...
- Phase-driven execution may also generate declarative prompt artifacts at `out/<CARD>/prompts/<prompt_alias>.md` when the current phase declares `outputs.prompts`.
...
- Legacy prompt artifacts may coexist with `out/<CARD>/prompts/` while compatibility support remains active.
